...
local OptionsToCar = {
        ...
        },
        {
                    ...
                    groups = Config.JobName,
                    canInteract = function()
                         return  onDuty and carout and  not balik and BoxCount  >= 1
                    end,
                    onSelect = function(data)
                    Debug(BoxCount)
                         balik = false
...
function StartDeliveryJob()
        ...
        local blip = AddBlipForCoord(randomLocation)
         ...
         AddTextComponentString(locale('Destination'))
         ...
         local pedModel = GetHashKey(Config.PedList[math.random(#Config.PedList)])
                        ...
                        local ped = CreatePed(4, pedModel, randomLocation.x, randomLocation.y, randomLocation.z, randomLocation.w, 1, 1)
                       ...
                       groups = Config.JobName,
                        label = locale('GiveMail'),
                         ...
                         onSelect = function(data)
                             ...
                             TriggerServerEvent('HS-GoPostalJob:Pay')
                             if BoxCount >= 1 then
                                 Debug(BoxCount)
                                 Wait(2500)
...
function CreatePeds()
    for i, pedData in ipairs(Config.PedSpawn) do
        local model = GetHashKey(pedData.model)
        RequestModel(model)
        while not HasModelLoaded(model) do
            Citizen.Wait(10)
        end
        local ped = CreatePed(26, model, pedData.coords.x, pedData.coords.y, pedData.coords.z, pedData.coords.w, false, false)
